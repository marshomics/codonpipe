...
def render_optimization_agreement_vs_genome(
    output_dir: Path,
    sample_id: str,
    ...
    Same layout as render_optimization_agreement, but the comparison anchor
    is the genome-mean RSCU rather than the RP set. Answers "if I were
    just matching the genome's overall codon distribution, which AAs
    would Mahal-style optimization route me to a different codon?".
    """
    plt = _plt()
    if plt is None or summary_g is None or summary_g.empty:
        return (None, None)

    fams = summary_g.copy().sort_values(
    ...
    ypos = np.arange(n)[::-1]
    cmap = plt.get_cmap("Reds")
    max_shift = max(0.01, fams["max_codon_w_shift_vs_genome"].max(skipna=True) or 0.01)
    ...
    for i, (_, r) in enumerate(fams.iterrows()):
        ...
        ax.text(-0.02, y, f"{r['family']} ({r['amino_acid']})",
                ha="right", va="center", fontsize=8, fontweight="bold",
                transform=ax.get_yaxis_transform())
        # Genome-most-frequent box
        ax.add_patch(plt.Rectangle((0.02, y - 0.35), 0.18, 0.7,
                                    color="#7f7f7f", alpha=0.85,
                                    transform=ax.get_yaxis_transform(), clip_on=False))
        ax.text(0.11, y, f"genome: {r['genome_optimal_codon']}",
                ha="center", va="center", fontsize=8, color="white",
                fontweight="bold",
                transform=ax.get_yaxis_transform())
        # Mahal-optimal box
        ...
        ax.text(0.43, y, symbol, ha="center", va="center",
                ...
                transform=ax.get_yaxis_transform())
        # Shift magnitude bar
        shift_norm = (r["max_codon_w_shift_vs_genome"] / max_shift) if max_shift > 0 else 0
        ax.add_patch(plt.Rectangle(
            (0.48, y - 0.3), 0.4 * shift_norm, 0.6,
            color=cmap(0.3 + 0.7 * shift_norm),
            transform=ax.get_yaxis_transform(), clip_on=False,
        ))
        ax.text(0.89, y, f"max Δw = {r['max_codon_w_shift_vs_genome']:.2f}",
                ha="left", va="center", fontsize=7,
                transform=ax.get_yaxis_transform())
    ...
    n_disagree = int((~fams["agree"]).sum())
    ax.set_title(
        f"Per-AA-family optimal codon: genome-mean vs Mahal-cluster — {sample_id}\n"
        f"{n_disagree} of {n} families would receive a different codon under "
        f"Mahal-style optimization than under naive genome-matching",
        fontsize=11, fontweight="bold",
    )

    out_dir = Path(output_dir)
    png = out_dir / f"{sample_id}_optimization_agreement_vs_genome.png"
    svg = out_dir / f"{sample_id}_optimization_agreement_vs_genome.svg"
    fig.savefig(png, dpi=300, bbox_inches="tight")
    fig.savefig(svg, bbox_inches="tight")
    plt.close(fig)
    return (png, svg)
...
def render_optimization_gain_vs_genome(
    output_dir: Path,
    sample_id: str,
    ...
    *,
    ...
    """Per-gene CAI gain from Mahal- vs genome-style codon optimization.

    Three panels mirroring render_optimization_gain but with the genome
    reference frame as the comparison anchor:
      A. Scatter of cai_genome vs cai_mahal coloured by membership_score
         when available; diagonal line marks equal performance.
      B. Histogram of (cai_mahal − cai_genome) annotated with the fraction
         of genes that improve under Mahal-style optimization.
      C. Bar chart of the top-N most-improved genes.

    The "gain over genome" axis quantifies how much better synthesis aimed
    at the Mahal cluster would translate compared with synthesis that just
    matches the genome's overall codon distribution.
    """
    plt = _plt()
    if plt is None or three_way_cai is None or three_way_cai.empty:
        return (None, None)
    needed = {"cai_genome", "cai_mahal", "gain_mahal_vs_genome"}
    if not needed.issubset(set(three_way_cai.columns)):
        return (None, None)

    df = three_way_cai.dropna(subset=["cai_genome", "cai_mahal"]).copy()
    if df.empty:
        return (None, None)
    ...
    if color_col == "membership_score":
        ...
        sc = axA.scatter(
            df["cai_genome"], df["cai_mahal"],
            c=df["membership_score"], cmap="viridis", s=12,
        ...
        axA.scatter(df.loc[~in_set, "cai_genome"], df.loc[~in_set, "cai_mahal"],
                    s=10, alpha=0.55, color="#888888", edgecolor="none",
                    label=f"out of cluster (n={int((~in_set).sum())})")
        axA.scatter(df.loc[in_set, "cai_genome"], df.loc[in_set, "cai_mahal"],
                    s=18, alpha=0.85, color="#2ca02c",
                    edgecolor="black", linewidth=0.3,
                    label=f"in Mahal cluster (n={int(in_set.sum())})")
        axA.legend(loc="best", fontsize=7, frameon=False)
    else:
        axA.scatter(df["cai_genome"], df["cai_mahal"],
                    s=10, alpha=0.5, color="#1f77b4")

    lo = float(min(df["cai_genome"].min(), df["cai_mahal"].min()))
    hi = float(max(df["cai_genome"].max(), df["cai_mahal"].max()))
    axA.plot([lo, hi], [lo, hi], color="black", linewidth=0.6, linestyle="--",
             alpha=0.6, label="cai_genome = cai_mahal")
    axA.set_xlabel("CAI (genome-derived w-values)")
    axA.set_ylabel("CAI (Mahal-cluster-derived w-values)")
    axA.set_title("A. Per-gene CAI: which reference fits better?", fontsize=10)
    for s in ("top", "right"):
        axA.spines[s].set_visible(False)

    # Panel B: gain distribution
    gains = df["gain_mahal_vs_genome"].dropna()
    if len(gains) > 0:
        axB.hist(gains, bins=40, color="#2ca02c", edgecolor="white", linewidth=0.4)
        ...
        axB.text(
            0.02, 0.98,
            f"{frac_pos:.1f}% of genes benefit from\nMahal-style optimization\nover naive genome-matching",
            transform=axB.transAxes, ha="left", va="top",
            fontsize=8,
            bbox=dict(facecolor="white", alpha=0.85, edgecolor="none"),
        )
        axB.set_xlabel("CAI gain from Mahal vs genome\n(cai_mahal − cai_genome)")
        axB.set_ylabel("Number of genes")
        axB.set_title("B. Distribution of optimization gain over genome", fontsize=10)
        axB.legend(loc="upper right", fontsize=8, frameon=False)
        for s in ("top", "right"):
            axB.spines[s].set_visible(False)

    # Panel C: top-N most-improved genes
    df_sorted = df.dropna(subset=["gain_mahal_vs_genome"]).sort_values(
        "gain_mahal_vs_genome", ascending=False,
    ).head(top_n)
    if not df_sorted.empty:
        ...
        axC.set_title(
            f"C. Top {len(df_sorted)} genes most improved by Mahal-vs-genome optimization",
            fontsize=10,
        )
    ...
    fig.suptitle(
        f"Codon-optimization gain vs genome — {sample_id}\n"
        "Mahal-cluster-derived optimal codons vs genome-most-frequent codons",
        fontsize=12, fontweight="bold",
    )

    out_dir = Path(output_dir)
    png = out_dir / f"{sample_id}_optimization_gain_vs_genome.png"
    svg = out_dir / f"{sample_id}_optimization_gain_vs_genome.svg"
    fig.savefig(png, dpi=300, bbox_inches="tight")
    fig.savefig(svg, bbox_inches="tight")
    plt.close(fig)
    return (png, svg)
