...
11b. **Codon pair bias** (Coleman et al. 2008) — codon-pair scores `CPS = ln(observed/expected)`, where the expected dicodon count is normalised for both single-codon and amino-acid-pair (dipeptide) usage, so CPB is independent of single-codon usage by construction. Emits the host codon-pair-score table (the reference a codon-pair-aware optimizer needs), per-gene codon pair bias, a bridge (codon-junction) dinucleotide observed/expected table — the leading non-translational explanation for CPB, so it is reported alongside as a confounder check (Tulloch et al. 2014; Kunec & Osterrieder 2016) — a genome-level summary, and a high-vs-low expression CPB comparison when expression tiers are available. CPB is emitted as a descriptive statistic, not as direct evidence of translational selection. Output goes to `codon_usage/codon_pair/`; skip with `--skip-codon-pair`.
...
- Pairwise Wilcoxon rank-sum tests per codon for each amino acid family (requires >=5 samples per group). The per-family TSVs (`{metadata}_{aa}_wilcoxon.tsv`) carry a family-local BH correction (`corrected_p_value_family`); the combined table (`{metadata}_all_wilcoxon.tsv`) additionally carries a genuine global BH correction across the full (codon × group-pair) test set (`corrected_p_value_global` / `significant_global`) — use the global column when asking "across all codons, which differ".
